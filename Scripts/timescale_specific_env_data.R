...
avg_growth_sync_timescale_env <- inner_join(timescale_specific_env, avg_sync_standardband, by=join_by(window_year == year, band == interval))

timescale_avg_ppt_avg_sync<- ggplot(data = avg_growth_sync_timescale_env, aes(x = window_avg_ppt, y = avg_sync, color = band)) +
  geom_point(data = avg_growth_sync_timescale_env, aes(x = window_avg_ppt, y = avg_sync, color = band), alpha = 0.5) +
...
timescale_avg_tmin_avg_sync<- ggplot(data = avg_growth_sync_timescale_env, aes(x = window_avg_tmin, y = avg_sync, color = band)) +
